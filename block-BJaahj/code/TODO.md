...
function objOfMatches(array1, array2, callback) {
  return array1.reduce((acc,cv,ci)=>{
    if(callback(cv) === array2[ci]){
      acc[cv] = array2[ci];
    }
    return acc;
  },{})
...
function multiMap(arrVals, arrCallbacks) {
  return arrVals.reduce((acc,cv)=>{
    acc[cv] = arrCallbacks.map(elem => elem(cv));
    return acc;
  },{})
}
...
//   return array1.reduce((acc,array1Cv,ci)=>{
//      acc[array1Cv] = callback.reduce((cbAcc,callbackCv)=>{
//       cbAcc = callbackCv(cbAcc);
//       return cbAcc;
//     },array1Cv)
//     return acc;
//   },{})
...
function objOfMatchesWithArray(array1, array2, callback) {
  return array1.reduce((acc,array1Cv,ci)=>{
     if(callback.reduce((cbAcc,callbackCv)=>{
      cbAcc = callbackCv(cbAcc);
      return cbAcc;
    },array1Cv) === array2[ci]){
      acc[arra1Cv] = array2[ci];
    }
    return acc;
  },{})
}
...
function objectWithArrayValues(array1,callback) {
  array1.reduce((acc,cv)=>{
    acc[cv] = callback.reduce((callbackAcc,callbackCv)=>{
      callbackAcc.push(cacllbackCv(cv));
      return callbackAcc;
    },[])
    return acc;
  },{})
